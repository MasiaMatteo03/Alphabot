from crypt import methods
import RPi.GPIO as GPIO
import time as T
from flask import Flask, app, render_template, url_for, redirect, request, make_response

class AlphaBot(object):
    
    def __init__(self, in1=13, in2=12, ena=6, in3=21, in4=20, enb=26):
        self.IN1 = in1
        self.IN2 = in2
        self.IN3 = in3
        self.IN4 = in4
        self.ENA = ena
        self.ENB = enb
        self.PA  = 41  #left engine
        self.PB  = 38  #right engine

        GPIO.setmode(GPIO.BCM)
        GPIO.setwarnings(False)
        GPIO.setup(self.IN1, GPIO.OUT)
        GPIO.setup(self.IN2, GPIO.OUT)
        GPIO.setup(self.IN3, GPIO.OUT)
        GPIO.setup(self.IN4, GPIO.OUT)
        GPIO.setup(self.ENA, GPIO.OUT)
        GPIO.setup(self.ENB, GPIO.OUT)
        self.PWML = GPIO.PWM(self.ENA,500)
        self.PWMR = GPIO.PWM(self.ENB,500)
        self.PWML.start(self.PA)
        self.PWMR.start(self.PB)
        self.stop()

    def forward(self, time_ms = 1000):  #that function move the alphabot forward for {time_ms} milliseconds
        print("The Alphabot moves forward...")
        

        self.PWML.ChangeDutyCycle(self.PA)      #setting the engines
        self.PWMR.ChangeDutyCycle(self.PB)
        GPIO.output(self.IN1, GPIO.LOW)
        GPIO.output(self.IN2, GPIO.HIGH)
        GPIO.output(self.IN3, GPIO.HIGH)
        GPIO.output(self.IN4, GPIO.LOW)

        T.sleep(time_ms/1000)
        self.stop()


    def backward(self, time_ms = 1000):        #this funtion move the alphabot backward for {time_ms} milliseconds
        print("The Alphabot moves backward...")

        self.PWML.ChangeDutyCycle(self.PA)      #setting the engines
        self.PWMR.ChangeDutyCycle(self.PB)
        GPIO.output(self.IN1, GPIO.HIGH)
        GPIO.output(self.IN2, GPIO.LOW)
        GPIO.output(self.IN3, GPIO.LOW)
        GPIO.output(self.IN4, GPIO.HIGH)

        T.sleep(time_ms/1000)
        self.stop()

    def left(self, time_ms = 1000):        #this function turn the alphabot left for {time_ms} milliseconds
        print("The Alphabot moves left...")
        
        self.PWML.ChangeDutyCycle(self.PA)      #setting the engines
        self.PWMR.ChangeDutyCycle(self.PB)
        GPIO.output(self.IN1, GPIO.HIGH)
        GPIO.output(self.IN2, GPIO.LOW)
        GPIO.output(self.IN3, GPIO.HIGH)
        GPIO.output(self.IN4, GPIO.LOW)

        T.sleep(time_ms/1000)
        self.stop()

    def right(self, time_ms = 1000):       #this function turn the alphabot right for {time_ms} milliseconds
        print("The Alphabot moves right...")
        
        self.PWML.ChangeDutyCycle(self.PA)      #setting the engines
        self.PWMR.ChangeDutyCycle(self.PB)
        GPIO.output(self.IN1, GPIO.LOW)
        GPIO.output(self.IN2, GPIO.HIGH)
        GPIO.output(self.IN3, GPIO.LOW)
        GPIO.output(self.IN4, GPIO.HIGH)

        T.sleep(time_ms/1000)
        self.stop()

    def stop(self, time_ms = 0):        #this function stops the alphabot for {time_ms} milliseconds
        print("The Alphabot stops...")

        self.PWML.ChangeDutyCycle(0)
        self.PWMR.ChangeDutyCycle(0)
        GPIO.output(self.IN1, GPIO.LOW)
        GPIO.output(self.IN2, GPIO.LOW)
        GPIO.output(self.IN3, GPIO.LOW)
        GPIO.output(self.IN4, GPIO.LOW)

        T.sleep(time_ms/1000)

            
    def set_pwm_a(self, value):
        if (value >= 0) and (value <= 100):
            GPIO.output(self.IN1, GPIO.HIGH)
            GPIO.output(self.IN2, GPIO.LOW)
            self.PWML.ChangeDutyCycle(value)
        elif (value < 0) and (value >= -100):
            GPIO.output(self.IN1, GPIO.LOW)
            GPIO.output(self.IN2, GPIO.HIGH)
            self.PWML.ChangeDutyCycle(0 - value) 

    def set_pwm_b(self, value):
        if (value >= 0) and (value <= 100):
            GPIO.output(self.IN1, GPIO.HIGH)
            GPIO.output(self.IN2, GPIO.LOW)
            self.PWML.ChangeDutyCycle(value)
        elif (value < 0) and (value >= -100):
            GPIO.output(self.IN1, GPIO.LOW)
            GPIO.output(self.IN2, GPIO.HIGH)
            self.PWML.ChangeDutyCycle(0 - value)   
        
    def set_motor(self, left, right):       #this function sets engines
        if (right >= 0) and (right <= 100):
            GPIO.output(self.IN1, GPIO.HIGH)
            GPIO.output(self.IN2, GPIO.LOW)
            self.PWML.ChangeDutyCycle(right)
        elif (right < 0) and (right >= -100):
            GPIO.output(self.IN1, GPIO.LOW)
            GPIO.output(self.IN2, GPIO.HIGH)
            self.PWML.ChangeDutyCycle(0 - right)
        if (left >= 0) and (left <= 100):
            GPIO.output(self.IN3, GPIO.HIGH)
            GPIO.output(self.IN4, GPIO.LOW)
            self.PWMR.ChangeDutyCycle(left)
        elif (left < 0) and (left >= -100):
            GPIO.output(self.IN3, GPIO.LOW)
            GPIO.output(self.IN4, GPIO.HIGH)
            self.PWMR.ChangeDutyCycle(0 - left)


Ab = AlphaBot()
app = Flask(__name__)

DR = 16
DL = 19

GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)
GPIO.setup(DR, GPIO.IN, GPIO.PUD_UP)
GPIO.setup(DL, GPIO.IN, GPIO.PUD_UP)

@app.route("/api/v1/sensors/obstacles", methods=['GET'])
def sensors_status():
    render_template('sensor_status.html')

    DR_status = GPIO.input(DR)
    DL_status = GPIO.input(DL)

    json = "{" + f'"right":{DR_status},' + f'"left":{DL_status}' + "}"
        
    print(json)
    return json
    

@app.route(f"/api/v1/motors/both", methods=['GET'])
def motors():
   #?pwmL={PWML}&pwmR={PWMR}&time={time} 
    pwmL=int(request.args['pwmL'])
    pwmR=int(request.args['pwmR'])
    time=int(request.args['time'])
    
    render_template("motors.html")
    print(pwmL, pwmR, time)

    Ab.set_pwm_a(pwmL)
    Ab.set_pwm_b(pwmR)
    Ab.forward(time)

if __name__ == '__main__':
    app.run(debug=True, host='192.168.0.133')